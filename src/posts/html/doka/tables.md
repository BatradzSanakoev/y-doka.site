...
[Сниппет с таблицей расписания поездов на странице поисковой выдачи Google](/assets/images/posts/tables/table1.jpg)
...
Задать границы элементам `<tr>`, `<thead>`, `<tfoot>` и `<tbody>` нельзя, поэтому задавайте их селекторам `table`, `th` или `td`.
...
🛠 Частый дизайнерский приём — подсветка строк таблицы через одну.

Например, сделаем каждую вторую строку с серым фоном. Для этого понадобится всего одно CSS-правило:
...
Если сайт должен хорошо работать в старых браузерах обязательно проверьте поддержку `sticky` на [сайте CanIuse](https://caniuse.com/css-sticky)
